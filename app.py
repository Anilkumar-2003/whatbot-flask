...
VERIFY_TOKEN = os.getenv("VERIFY_TOKEN", "my_verify_token")  # set same in Meta dashboard
...
def upsert_user_by_phone(phone, data):
    # data: dict with name,email,class,school,role
...
def record_answer(user_id, question_id, selected_option):
    ...
    # update user score (count correct answers)
...
def teacher_summary_text():
    # students who have answers count >= total_questions
    tq = total_questions()
    pipeline = [
        {"$match": {"role": "student"}},
        {"$lookup": {
        ...
        {"$project": {"name": 1, "email": 1, "score": 1, "answers_count": {"$size": "$answers"}}},
...
def webhook():
    ...
    # navigate payload: entry -> changes -> value -> messages
    try:
        ...
        if not messages:
            return jsonify({}), 200
        msg = messages[0]
        phone = msg.get("from")  # e.g. "919166082886"
        text = ""
        if "text" in msg and "body" in msg["text"]:
            text = msg["text"]["body"].strip()
        else:
            send_text(phone, "Sorry, I only accept text messages right now.")
            return jsonify({}), 200
        ...
        if state == "awaiting_school":
            ...
            send_text(phone, "Are you a 'student' or 'teacher'? Please type student or teacher.")
            return jsonify({}), 200

        if state == "awaiting_role":
            if text_l not in ["student","teacher"]:
                send_text(phone, "Please type exactly 'student' or 'teacher'.")
                return jsonify({}), 200
            temp = session.get("temp_data", {})
            temp["role"] = text_l
            # create/update user
            user_doc = upsert_user_by_phone(phone, temp)
            create_or_update_session(phone, "awaiting_start_confirmation", temp_data=temp, user_id=user_doc.get("_id"))
            send_text(phone, "Registration saved. Type YES to start the quiz (students) or type YES to view results (teachers). Type NO to cancel.")
            return jsonify({}), 200
        ...
        if state == "awaiting_start_confirmation":
            if text_l in ["yes","y"]:
                ...
                if role == "teacher":
                    ...
                    # reset user's previous answers and score (optional)
                    ...
                    if not q:
                        ...
                        clear_session(phone)
                        return jsonify({}), 200
                    qtext = format_question(q, 1)
                    send_text(phone, qtext)
                    return jsonify({}), 200
            elif text_l in ["no","n"]:
                send_text(phone, "Cancelled. Send 'Hi' to start again.")
                clear_session(phone)
                return jsonify({}), 200
            else:
                send_text(phone, "Please reply YES or NO.")
                return jsonify({}), 200

        # quiz answering
        if state == "in_quiz":
            if text_l.upper() not in ["A","B","C","D"]:
                send_text(phone, "Reply with A, B, C or D for your answer.")
                return jsonify({}), 200
            user = find_user_by_phone(phone)
            user_id = user.get("_id")
            ...
            if not q:
                send_text(phone, "Question not found. Aborting.")
                clear_session(phone)
                return jsonify({}), 200
            # record answer
            is_correct = record_answer(user_id, q["_id"], text_l.upper())
            if is_correct:
                send_text(phone, "Correct! ✅")
            else:
                send_text(phone, f"Incorrect. Correct answer: {q['correct_option']}")
            send_text(phone, f"Explanation: {q.get('explanation','')}")
            # next question
            next_idx = q_index + 1
            ...
            if next_idx >= total:
                ...
                create_or_update_session(phone, "in_quiz", temp_data=session.get("temp_data", {}), current_q_index=next_idx, user_id=user_id)
                q2 = get_question_by_index(next_idx)
                send_text(phone, format_question(q2, next_idx+1))
                return jsonify({}), 200

        send_text(phone, "Unknown state. Send 'Hi' to begin.")
        return jsonify({}), 200
